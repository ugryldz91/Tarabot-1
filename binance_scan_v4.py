...
def check_conditions(df: pd.DataFrame) -> Tuple[bool, float]:
    ...
    # Hacim koşulu: "son gün" = son kapanmış gün
    if i_cl < 4 or pd.isna(volume.iloc[i_cl]):
        return (False, float(last_rsi) if not pd.isna(last_rsi) else float("nan"))

    prev3 = volume.iloc[i_cl-3:i_cl]  # önceki 3 kapanmış gün
    if prev3.isna().any():
        return (False, float(last_rsi) if not pd.isna(last_rsi) else float("nan"))
