...
- BTC/ETH için son kapanan mum RSI(14) mesaj başında gösterilir
"""
...
# ---------- Condition Check (closed candle guaranteed) ----------
def check_conditions(df: pd.DataFrame) -> Tuple[bool, float]:
    ...
    # Son mum kapanmadıysa bir önceki mumu değerlendir
    ...
    # Hacim: önceki 3 gün ortalaması (kapanan mumdan önceki günler)
...
def format_message(pairs: List[Tuple[str,float]], scanned: int, majors: Optional[Dict[str, Optional[float]]] = None) -> str:
    ...
    if majors:
        ...
        for sym in ["BTCUSDT", "ETHUSDT"]:
            v = majors.get(sym)
            lines.append(f"- {sym}: RSI={v if v is not None else 'N/A'}")
